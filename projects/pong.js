...
function preload() {
    soundFormats('wav');
    dingSound = loadSound('/resources/ding.wav');
}
...
function setup() {
    ...
    //create speed max slider
    ballsNumSlider = createSlider(1, 100, 15);
...
function draw() {
    ...
    rectMode[RADIUS];
    fill(0);
    rect(mouseX - 100, height - 60, 200, 20);
    infoOverlays();
}
...
function drawSphere() {
    ...
    this.xMark = 1;
    this.yMark = 1;
    this.constR = Math.floor(random(0, 255));
    this.constG = Math.floor(random(0, 255));
    this.constB = Math.floor(random(0, 255));
    ...
    this.move = function () {
        ...
        if (pause == false) {
            ...
            if ((this.y < (height - 60 - radius)) && (this.x > (mouseX - 100) || (this.x < mouseX + 100))) {
                this.y = radius;
                this.yMark = -this.yMark;
                this.ySpeed = this.constRandSpeed * this.yMark;
                this.change = true;
